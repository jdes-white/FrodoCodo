 ...
 * pipeline from @frodocodo/ledger — not a hand-authored fixture. This is
 * what makes `pnpm db:seed` a genuine (if abbreviated) run of Slice 5's
 * sync pipeline against Slice 1-4's schema.
 *
...
import { MockProvider, MOCK_INSTITUTIONS } from "../../providers/src/index.js";
import { normalizeMerchant, detectTransferPairs, detectRefunds } from "../../ledger/src/index.js";
import { todayUTC } from "../../shared/src/index.js";
...
async function main() {
  console.log("Wiping existing demo households...");
  ...
  console.log("Creating household and users...");
  ...
  console.log("Creating budget buckets and categories...");
  ...
  console.log("Creating this period's budget allocations...");
  const asOf = todayUTC();
  ...
  console.log(`Budget period ${budgetPeriod.startDate.toISOString().slice(0, 10)} → ${budgetPeriod.endDate.toISOString().slice(0, 10)}`);

  console.log("Connecting mock institutions and syncing transactions...");
  ...
  console.log(`Inserting ${staged.length} transactions...`);
  ...
  console.log("Reconciling transfers and refunds via @frodocodo/ledger...");
  ...
  console.log(`Member login: ${DEMO_MEMBER_EMAIL} / ${DEMO_PASSWORD}`);
}
