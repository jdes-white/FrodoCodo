...
- `BasiqProvider` (Task 7A, corrected in Task 7A.1/7A.2,
  `packages/providers/src/basiq/`) — a real adapter for **CBA and Virgin
  Money only** (Amex remains out of scope — see below), with a real
  connect-flow UI wired up in `apps/web` as of Task 7C (Settings → Connect
  an institution → hosted Consent UI → callback → sync). **Implemented but
  ...
  design, the live-connection flow, and every item that could not be
  verified against live Basiq documentation from the environment this was
  built in.
