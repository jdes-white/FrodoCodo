 ...
 */
export function AddCommitmentCard() {
    ...
    return (
      <button type="button" onClick={() => setExpanded(true)} className="block w-full text-left">
        <Card padding="p-3" className="text-center text-sm font-semibold">
          <span style={{ color: "var(--color-accent)" }}>+ Add a commitment</span>
        </Card>
      </button>
    );
  }

  return <ExpandedAddCommitmentCard onDone={() => setExpanded(false)} />;
}

function ExpandedAddCommitmentCard({ onDone }: { onDone: () => void }) {
