...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // getHousehold is cache()-deduped (lib/household.ts) — most primary
  // pages also fetch the household via getBudgetSnapshot, and this way
  // that's one DB round-trip per request instead of two.
  const [user, household] = await Promise.all([getCurrentUser(session), getHousehold(session.householdId)]);

  return (
    <div className="flex min-h-screen flex-col sm:flex-row">
      <NavBar />
      <SwipeNavigation className="flex-1 pb-20 sm:pb-0">
        <header
